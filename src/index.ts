...
function main() {
	...
	numberBox.addEventListener("input", (event: Event) => {
		const text =
			(event?.target as HTMLInputElement).value.replace(/[^0-9.]+[0]*/g, "") ??
			"";
...
function drawBbox(ctx: CanvasRenderingContext2D, bbox: any) {
	...
	ctx.strokeRect(bbox.position.x, bbox.position.y, bbox.width, bbox.height);
}

function drawTitleText(ctx: CanvasRenderingContext2D, half: any, center: Box) {
	...
	ctx.fillText(title, width, 115, ctx.canvas.width * 1);
}
...
function drawCenterLine(ctx: CanvasRenderingContext2D, half: Box, center: Box) {
	ctx.moveTo(center.width, center.height + (half.height - ctx.lineWidth / 2));
	ctx.lineTo(center.width, center.height - (half.height - ctx.lineWidth / 2));
}
...
function draw2(
	...
	ctx.moveTo(center.width, center.height - half.height * 0.3 * flip.y);
	ctx.lineTo(
		center.width + (half.width - ctx.lineWidth / 2) * flip.x,
...
function draw9(
	...
	ctx.lineTo(center.width, center.height - half.height * 0.3 * flip.y);
}
